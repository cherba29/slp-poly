...
std::ostream& operator<<(std::ostream& os, const eval::Instruction<F>& instr) {
  os << std::setw(10) << instr.getType().toString() << std::setw(4)
...
class InstructionPrinter {
 ...
 public:
  ...
  void operator()(const eval::Instruction<F>& instr) {
    ...
    if (ann.length() > 0) {
      if (ann.length() > 300) {
        os_ << ann.substr(0, 300) << "..." << std::endl;
      } else {
        os_ << ann << std::endl;
      }
    }
    os_ << std::setw(6) << count_++ << std::setw(10)
        << instr.getType().toString() << std::setw(10)
        << ann_.getDataName(instr.getDestination()) << " ("
        << instr.getDestination() << "), ";
