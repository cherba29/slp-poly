...
class Evaluator {
 ...
 public:
  Evaluator(int nVars, int dataSize,
      ...
      : nVars_(nVars),  // Homogenized version effectively has n+1 variables
        ...
        dataSize_(dataSize),      // Data block already includes space for
                                  // homogenizing var
        data_(new F[dataSize_]),  // actually allocate data block, code refers
                                  // to it by index
        code_(code),              // Array of instructions to execute
        ann_(ann)
  // result_(data_+(code_->back().getDestination())) // Result is produced by
  // last instruction
  {
  ...
  template <typename T>
  void traceWith(T& f) {
    typename std::vector<Instruction<F> >::const_iterator instr =
        code_->begin();
   ...
   */
  const CodeAnnotation& getAnnotation() const { return *ann_; }
  ...
  for (int ip = 0; instr != code_->end(); ip++, ++instr) {
    ...
    switch (instr->getType().getId()) {
      ...
      case InstructionEnum::SETM2P: {
        math::field::util::setFill2P(&dest, instr->getConstant(),
                                     instr->getSource1());
        // LDBG_ << "loc[" << instr->getDestination() << "] = " << dest << " <--
        // " << instr->getSource1();
        break;
      ...
      case InstructionEnum::SETM: {
        math::field::util::setFill(&dest, instr->getConstant(),
                                   instr->getSource1());
        // LDBG_ << "loc[" << instr->getDestination() << "] = " << dest << " <--
        // " << instr->getSource1();
        break;
...
//           instr->getSource2() // size, where layout size is
//           2^(ceil(log(size)))
//         );
//         break;
//       }
...
//       }
//       default: {
//         LERR_ << "Unimplemented instruction: " <<
//         instr->getType().toString(); BOOST_ASSERT(false);
//       }
//     }
//   }
