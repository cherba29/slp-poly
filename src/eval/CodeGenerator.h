...
/**
 * @brief Main responsibility of this class to construct executable instructions
 * from context which is usually parsed from given input file. CodeGenerator is
 * a context::Value visitor implementing context::Processor. Given
 * context::InterpContext it produces eval::Instruction stream, from which
 *        eval::Evaluator can be constucted. It also produces code and data
 * anotation for debugging and code tracing.
 * @code
 *   // Create context
 *   boost::shared_ptr<context::InterpContext> ctxtPtr(new
 ...
 *   boost::shared_ptr<eval::Evaluator<Field> > evalPtr =
 * cg.getEvaluator<Field>(); if (!evalPtr) { LERR_ << "Not able to build
 * evaluator from parsed in context";
 *   }
 * @endcode
...
template <typename F>
class CodeGenerator
    : private context::Processor {  // not publicly convertable to processor
 ...
 public:
  ...
  boost::shared_ptr<Evaluator<F> > getEvaluator() const {
    ...
    return boost::shared_ptr<Evaluator<F> >(new Evaluator<F>(
        nVars, this->alloc.getMaxAllocSize(), codePtr_, annPtr_));
  }
};
