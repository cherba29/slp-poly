...
bool validatePolynomial(
    ...
    boost::shared_ptr<algebra::PartialPolynomial<F> const> ppol, E& evaluator) {
...
template <typename F>
template <typename E>
boost::shared_ptr<algebra::Polynomial<F> const> Interpolator<F>::interpolate(
  ...
  for (int i = 0; i < nVars; ++i) {
    ...
    LDBG_ << "Random evaluation point " << i << ". "
          << evaluator.getAnnotation().getDataName(i) << " = " << evalPoints[i];
  }
  ...
  if (nVars == 1) {                 // Perform univariate interpolation
    ...
    PartialPolynomialPtr univPartialPol(new algebra::PartialPolynomial<F>(
        ...
        newtonInterpolateUnivariateEvaluator<F>(
            varMaxDegrees[0], evaluator.getAnnotation().getDataName(0),
            evaluator)));

    LAPP2_ << "Verifying stage " << verifyStage_ << " time(s)...";
    if (!detail::validatePolynomial<F, E>(verifyStage_, univPartialPol,
                                          evaluator)) {
  ...
  typedef std::multiset<boost::shared_ptr<algebra::PartialPolynomial<F> >,
  ...
  while (polPool.size() > 1) {  // Pool has more than one polynomial in it.
    ...
    for (; it != polPool.end(); ++it) {
      ...
      LAPP1_ << "  [" << ss.str() << "]. " << pol.getNumberOfTerms()
             << " terms "
    ...
    LAPP1_ << "Removed and merging two polynomials "
           << polA->getPolynomial()->getShortName() << " and "
           << polB->getPolynomial()->getShortName() << ". max size: "
           << polA->getPolynomial()->getNumberOfTerms() *
                  polB->getPolynomial()->getNumberOfTerms()
           << " ...";

    PartialPolynomialPtr merged =
        sparseInterpolateStage(evaluator, *polA, *polB);

    int nTerms = merged->getPolynomial()->getNumberOfTerms();
    long double prob = static_cast<long double>(maxTotalDegree) * nTerms *
                       (nTerms - 1) / (2 * (F::getSize() - 1));

    if (prob > 0.1) {
      LWRN_ << "Bound on probability of error is "
            << ((prob > 1.0) ? 1.0 : prob);
    } else {
...
Interpolator<F>::sparseInterpolateStage(
    E& evaluator, const algebra::PartialPolynomial<F>& ppolA,
  ...
  double firstAlgTime =
      complexity::Perf::predicted(firstAlgName, nTermsA, nTermsB - 1);
  LAPP1_ << "  Will be solving " << std::right << (nTermsB - 1)
         << " vand systems of size " << nTermsA << " x " << nTermsA
         << " with alg " << firstAlgName;
  ...
  double secondAlgTime =
      complexity::Perf::predicted(secondAlgName, nTermsB, nTermsA);
  ...
  double evalTime =
      complexity::Perf::predicted(complexity::AlgEnum::VAL_OF_EVAL, nTermsA, 1);

  LAPP1_ << "  Will be evaluating black box " << nTermsA
         << " times, predicted time " << evalTime;
  ...
  LAPP1_ << "  polA terms: " << nTermsA << " variables: (" << nVarsA << "): ["
         << polA.getShortName() << "]";
  LAPP2_ << "     indexes: [" << polAIdxStr.str() << "]";
  LAPP1_ << "  polB terms: " << nTermsB << " variables: (" << nVarsB << "): ["
         << polB.getShortName() << "]";
  ...
  for (int tB = 1; tB < nTermsB; ++tB) {
    ...
    for (int tA = 0; tA < nTermsA; ++tA) {
      ...
      coeffAGrid[tA << nTermsBPw2 | tB] =
          vanCoeffSetA[tA] * vanEntryInvSetA[tA];
    }
  }
  /*
  ...
  */
  ...
  LAPP1_ << "  solving " << std::setw(5) << std::right << nTermsA << " "
         << std::setw(4) << std::right << nTermsB << "x" << std::setw(4)
         << std::left << nTermsB << " vand systems";
  //  LAPP1_ << "  predicted time "
  ...
  LAPP1_ << "  Merged polynomial ... "
         << " Tot Deg = " << pol->getTotDegree()
         << " nTerms  = " << pol->getNumberOfTerms() << " in " << tm.elapsed()
         << " secs";
