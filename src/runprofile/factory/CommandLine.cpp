 ...
 * at the same time.
 */
inline bool both_options(const po::variables_map& vm, const char* opt1,
                         const char* opt2) {
  return (vm.count(opt1) && !vm[opt1].defaulted() && vm.count(opt2) &&
          !vm[opt2].defaulted());
}
 ...
 */
bool atLeastOne(const po::variables_map& vm, const char* options[],
 ...
 */
bool exclusiveOptions(const po::variables_map& vm, const char* options[],
                      unsigned int nOptions) {
  for (unsigned int i = 0; i < nOptions; i++) {
 ...
 */
bool conflicting_options(const po::variables_map& vm, const char opt1[],
                         const char* options[], unsigned int nOptions) {
  if (vm.count(opt1) && !vm[opt1].defaulted()) {
    for (unsigned int i = 0; i < nOptions; i++) {
      if (vm.count(options[i]) && !vm[options[i]].defaulted()) return true;
    }
  }
  return false;
}
 ...
 */
inline bool option_dependency(const po::variables_map& vm, const char* for_what,
              ...
              int)
{
  ...
  if ( pos != std::string::npos) {
    ...
    std::pair< ::logging::Module,::logging::Level> modLogLevel(moduleStr,
logLevelStr); if (modLogLevel.first == ::logging::Module::UNKNOWN) { throw
std::logic_error(std::string("Unknown module: ")+moduleStr);
    }
...
// typedef std::pair< ::logging::LogModuleEnum, ::logging::LogLevelEnum>
// ModLogLevel;
...
std::istream& operator>>(std::istream& is, ModLogLevel& modLogLevel) {
  ...
  if (pos != std::string::npos) {
    ...
    if (!module || module == ::logging::LogModuleEnum::UNKNOWN) {
      ...
      throw std::logic_error(std::string("Unknown module: [") + module_name +
                             "] use one of" + ss.str());
    }
    ...
    throw po::validation_error(po::validation_error::invalid_option_value, s,
                               "module logging level, expecting module=level");
  }
  return is;
};
...
std::unique_ptr<RunProfile> CommandLine::getRunProfile(int argc, char* argv[]) {
  // Boolean flags
  bool h_given = false;
  bool v_given = false;
  bool w_given = false;
  ...
  mainOptions.add_options()(
      ...
      "version,v", po::bool_switch(&v_given), "print version string")(
  ...
  genOptions.add_options()(
      "level,l", po::value<std::vector<ModLogLevel> >(&logLevels)->composing(),
      "<module name>=<log level>. Log levels:\n"
      "  trace - \tVery detailed program execution trace. Includes all other "
      "level.\n"
      "  debug - \tLess detailed than trace, used by key complex areas of "
      "code.\n"
      "  info  - \t(default) Application level/domain specific messages.\n"
      "  inf1  - \tMore detailed application messages, shows all info as "
      "well.\n"
      "  inf2  - \tEven more detailed application messages, shows all info and "
      "info1.\n"
      "  warning - \tWarnings.\n"
      ...
      "h -\t Thread id")("response-file", po::value<std::string>(),
      ...
      //        "Save trace, debug and error level messages into specified
      //        file.")
      //    ("runtime", po::value<std::string>(),
      //        "Save application level messages into specified file. Will
  ...
  interpOptions.add_options()("asmfile,a", po::value<std::string>(),
                              "Save compiled input into assembly file.")(
      "field,f",
      po::value<std::string>(&fieldOptionStr)
      ...
      "  <AB|AC|BC><32|64>       - \tAB,AC,BC use two hardcoded primes. eg. "
      "AC32 \n"
#ifdef FIELD_DYNAMIC64
      "  <integer 32/64-bit>     - \tSpecify any prime to use (slower than "
      "hardcoded)\n"
      ...
      "  <integer>          - \tSpecify any 32 bit prime to use (slower than "
      "hardcoded)\n"
      ...
      ("order,r", po::value<std::string>(&orderStr)->default_value("evalpts"),
       ...
       "  minterms - \tminimum number of terms first")(
          ...
          "  max_last - \tdeclared order but max in number of terms last (it "
          "comes for free)")(
          "random-seed", po::value<int>(),
          "Seed to use to initialize random number generator. "
          "Default is time.")("savefile,s", po::value<std::string>(),
                              "Save intermediate state into specified file.\n"
                              "if [filename] already exists, resume from the "
                              "state saved in that file.")(
          "univar,u",
          po::value<std::string>(&univarStr)->default_value("newton"),
          ...
          "  adapt - \tChoose fastest algorithm quad or fft for each matrix "
          "(default)")("verify-stage",
                       po::value<int>(&verifyStage)->default_value(10),
  ...
  hidden.add_options()("input-file,i", po::value<std::string>(),
                       "maple input file")
      //    ("output-file,o", po::value<std::string>(), "maple output file")
      ;
  // First and second arguments are treated as input and output files
  // respectively
  ...
  config_file_options.add(interpOptions).add(config).add(hidden);

  po::options_description visible("Allowed options");
  visible.add(mainOptions).add(genOptions);
  ...
  if (vm.count("response-file")) {
    ...
    if (!ifs) {
      throw std::logic_error(std::string("Could no open the response file ") +
                             filename);
    }
  ...
  //---------------------------------------------------------------
  // Verify options
  const char* mainOptionNames[] = {"version", "benchmark", "machine",
                                   "interpolate"};
  ...
  for (unsigned int i = 0; i < logTags.size(); ++i) {
    switch (logTags[i]) {
      case 'i':
        prof->addLogTag(logging::LogTagEnum::INDEX);
        break;
      case 't':
        prof->addLogTag(logging::LogTagEnum::TIME);
        break;
      case 'm':
        prof->addLogTag(logging::LogTagEnum::MODULE);
        break;
      case 'l':
        prof->addLogTag(logging::LogTagEnum::LEVEL);
        break;
      case 'n':
        prof->addLogTag(logging::LogTagEnum::FILELINE);
        break;
      case 'f':
        prof->addLogTag(logging::LogTagEnum::FUNCTION);
        break;
      case 'h':
        prof->addLogTag(logging::LogTagEnum::THREAD_ID);
        break;
      default:
        throw std::logic_error(std::string("Unknown log tag option: ") +
                               logTags[i]);
    }
  }

  // conflicting_options(vm, "help", help_conflicts,
  // sizeof(help_conflicts)/sizeof(char*)); conflicting_options(vm, "benchmark",
  // "version"); conflicting_options(vm, "benchmark", "interpolate");
  ...
  if (v_given) {
    ...
    if (iprof->getFieldOption() == FieldOptionEnum::UNKNOWN) {
      throw std::logic_error(std::string("Unknown field option: ") +
                             fieldOptionStr);
    }
