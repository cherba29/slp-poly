...
public:
  ...
  virtual long double solveFor(long double val) const {
    ...
    for (int i = 0; i < MAX_ITER; i++) {
      ...
      root -= dx;
      LTRC_ << i << "  root: " << root;
      if (root < xStart || root > xEnd) { return -2; } // root jumped out
  ...
  static long double intersectsAt(const Equation& a, const Equation& b) {
    ...
    if (i >= MAX_ITER) { // initial guess was too high
      xEnd = 1024;
      LTRC_ << "initial guess was too high, reducing lower bound, setting xEnd to " << xEnd;
...
public:
  LinearEquation() { coeffs_[0] = 0.0; coeffs_[1] = 0.0; }
