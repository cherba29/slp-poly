 ...
 *        Instantiation over different field is split in separate compilation units,
 *        this somewhat reduces compilation error messages since they do
 *        not repeat for each instantiation
 */
class Interpolation {
...
public:
  ...
  int runDynamic32(const boost::shared_ptr<context::InterpContext>& ctxtPtr, util::MultiIndexMap* m);
#endif
#ifdef FIELD_A32
  int runA32(const boost::shared_ptr<context::InterpContext>& ctxtPtr, util::MultiIndexMap* m);
#endif
#ifdef FIELD_B32
  int runB32(const boost::shared_ptr<context::InterpContext>& ctxtPtr, util::MultiIndexMap* m);
#endif
#ifdef FIELD_C32
  int runC32(const boost::shared_ptr<context::InterpContext>& ctxtPtr, util::MultiIndexMap* m);
#endif
#ifdef FIELD_AB32
  int runAB32(const boost::shared_ptr<context::InterpContext>& ctxtPtr, util::MultiIndexMap* m);
#endif
#ifdef FIELD_BC32
  int runBC32(const boost::shared_ptr<context::InterpContext>& ctxtPtr, util::MultiIndexMap* m);
#endif
#ifdef FIELD_AC32
  int runAC32(const boost::shared_ptr<context::InterpContext>& ctxtPtr, util::MultiIndexMap* m);
#endif
#ifdef FIELD_CRT32
  int runCRT32(const boost::shared_ptr<context::InterpContext>& ctxtPtr, util::MultiIndexMap* m);
#endif
#ifdef FIELD_DYNAMIC64
  int runDynamic64(const boost::shared_ptr<context::InterpContext>& ctxtPtr, util::MultiIndexMap* m);
#endif
#ifdef FIELD_A64
  int runA64(const boost::shared_ptr<context::InterpContext>& ctxtPtr, util::MultiIndexMap* m);
#endif
#ifdef FIELD_B64
  int runB64(const boost::shared_ptr<context::InterpContext>& ctxtPtr, util::MultiIndexMap* m);
#endif
#ifdef FIELD_C64
  int runC64(const boost::shared_ptr<context::InterpContext>& ctxtPtr, util::MultiIndexMap* m);
#endif
#ifdef FIELD_AB64
  int runAB64(const boost::shared_ptr<context::InterpContext>& ctxtPtr, util::MultiIndexMap* m);
#endif
#ifdef FIELD_BC64
  int runBC64(const boost::shared_ptr<context::InterpContext>& ctxtPtr, util::MultiIndexMap* m);
#endif
#ifdef FIELD_AC64
  int runAC64(const boost::shared_ptr<context::InterpContext>& ctxtPtr, util::MultiIndexMap* m);
#endif
#ifdef FIELD_CRT64
  int runCRT64(const boost::shared_ptr<context::InterpContext>& ctxtPtr, util::MultiIndexMap* m);
#endif
  ...
  int runWithField(
    ...
    {
      LAPP1_ << "Field element stack memory footprint is " << sizeof(F) << " bytes.";
    ...
    const boost::optional<std::string>& asmFileName = interpProfile_->getAsmFileName();
    if (asmFileName) {
    ...
    for (int i = 0; i < eval::InstructionEnum::NUM_ENUMS; ++i) {
      ...
      if (count > 0) {
        ...
        LAPP2_ << "  " << std::setw(7) << instrType << " - "
          << std::setw(5) << std::right << count << " times (" << std::right
          << std::setw(5) << std::fixed << std::setprecision(2) << count*100.0/totNumInstr << "%)";
      }
    }
    ...
    {  // -- check if evaluator is identically zero --------
      ...
      if (check >= NUM_CHECK_FOR_ZERO) {
        ...
        oss << "The input evaluates to zero (" << NUM_CHECK_FOR_ZERO << " times check). Exiting...";
        LERR_ << oss.str();
    ...
    // == Benchmark main used algorithms ==================
    {
      ...
      const double BENCHMARK_TIME_LIMIT = 2.0;
      {
        ...
        const complexity::Equation* eq = Perf::getEquation(AlgEnum::VAL_OF_VAND_QUAD_SETUP);
        if (eq) {
          LAPP1_ << "  equation " << *eq;
      ...
      {
        LAPP1_ << "Benchmarking " << AlgEnum::VAND_QUAD_SOLVE << " for " << BENCHMARK_TIME_LIMIT << " sec";
        Perf::exercise(AlgEnum::VAL_OF_VAND_QUAD_SOLVE,operation::vandermonde::QuadSolve<F>(),BENCHMARK_TIME_LIMIT);
        const complexity::Equation* eq = Perf::getEquation(AlgEnum::VAL_OF_VAND_QUAD_SOLVE);
        if (eq) {
          LAPP1_ << "  equation " << *eq;
      ...
      {
        LAPP1_ << "Benchmarking " << AlgEnum::VAND_FFT_SETUP << " for " << BENCHMARK_TIME_LIMIT << " sec";
        Perf::exercise(AlgEnum::VAL_OF_VAND_FFT_SETUP,operation::vandermonde::FFTSetup<F>(),BENCHMARK_TIME_LIMIT);
        const complexity::Equation* eq = Perf::getEquation(AlgEnum::VAL_OF_VAND_FFT_SETUP);
        if (eq) {
          LAPP1_ << "  equation " << *eq;
      ...
      {
        LAPP1_ << "Benchmarking " << AlgEnum::VAND_FFT_SOLVE << " for " << BENCHMARK_TIME_LIMIT << " sec";
        Perf::exercise(AlgEnum::VAL_OF_VAND_FFT_SOLVE,operation::vandermonde::FFTSolve<F>(),BENCHMARK_TIME_LIMIT);
        const complexity::Equation* eq = Perf::getEquation(AlgEnum::VAL_OF_VAND_FFT_SOLVE);
        if (eq) {
          LAPP1_ << "  equation " << *eq;
      ...
      {
        LAPP1_ << "Benchmarking " << AlgEnum::EVAL << " for " << BENCHMARK_TIME_LIMIT << " sec";
        Perf::exercise(AlgEnum::VAL_OF_EVAL,operation::eval::Evaluator<F>(bbeval),2.0);
        const complexity::Equation* eq = Perf::getEquation(AlgEnum::VAL_OF_EVAL);
        if (eq) {
          LAPP1_ << "  equation " << *eq;
    ...
    interp::Interpolator<F> interp(
      getVandermondeFactory<F>(),
      getInterpPolynomialOrder<F>(),
      interpProfile_->getPrunningChoice(),
      interpProfile_->getVerifyStage(),
    ...
    switch (interpProfile_->getVandermondeSolver().getId()) {
      case interp::VandSolveTypeEnum::QUAD:
        return  boost::shared_ptr<interp::VandSolverFactory<F> >(
            new interp::VandSolverFactory<F>(
                interp::VandSolveTypeEnum::QUAD));
      case interp::VandSolveTypeEnum::FFT:
        return  boost::shared_ptr<interp::VandSolverFactory<F> >(
            new interp::VandSolverFactory<F>(
                interp::VandSolveTypeEnum::FFT));
