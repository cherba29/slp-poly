...
struct ModOp {
  ...
  UIntType add(UIntType l, UIntType r, UIntType m) {
    UIntType val = m - l;
   ...
   */
  static __forceinline
  UIntType sub(UIntType l, UIntType r, UIntType m) {
    if (l >= r) { return l-r; }
    UIntType diff = (r - l);
  ...
  static __forceinline
  bool eq(UIntType l, UIntType r, UIntType m) {
    return (l%m == r%m);
  }
   ...
   */
  static __forceinline
  UIntType mul(UIntType l, UIntType r, UIntType m) {
    return (static_cast<typename UIntTypeInfo<UIntType>::unsigned_type2x>(l)*r) % m;
  }

  static __forceinline
  UIntType mul_add(UIntType l, UIntType r, UIntType a, UIntType m) {
   ...
   */
  static __forceinline
  UIntType sub_mul(UIntType l, UIntType r, UIntType a, UIntType m) {
    typename UIntTypeInfo<UIntType>::unsigned_type2x q
      = sub(l,r,m);
    q *= a;
    return q % m;
  }
   ...
   */
  static __forceinline
  UIntType inv(UIntType a, UIntType m) {
   ...
   */
  static __forceinline
  UIntType pow(UIntType left, UIntType right, UIntType m) {
    UIntType base = left;
    UIntType n = right;
    UIntType result = 1;
    while (1) {
   ...
   */
  static __forceinline
  UIntType pow2p(UIntType a, long p, UIntType m) {
    while (p--) {
      a = ModOp::mul(a, a, m);
    }
    return a;
   ...
   */
  static
  __forceinline UIntType sqrt2(UIntType a, UIntType mod) {
  ...
  static
  __forceinline UIntType sqrt(UIntType a, UIntType mod) {
    // Euler's criterion: root exists iff a^((mod-1)/2) % mod == 1
    const UIntType POS = (mod-1)>>1;
    if ( ModOp::pow(a, POS, mod) != 1 ) return 0;
    ...
    while (b != 1) {
      ...
      ASSERT1(m < r)(m)(r)(b)(t)(e)(q)(n)(y)(a)(mod)
        .msg("this condition should not occur if there exist a root for a");
...
public:
  ...
  /**
   * @brief Compute mixed radix representations of crts value using Garners algorithm.
   * Complexity of this algorithm is O(n^2) where n is number of moduli.
   *  if crts_i = X mod m_i, then
   *    X = rads_0 + rads_1*m_0+rads_2*m_0*m_1+...+rads_n*m_0...m_{n-1}
   * This algorithm uses precomputed mixedmultipliers see setPrimes().
   *
   * @param rads - (output) mixed radix represention of length n (constructor param)
  ...
  int mixedRadixToString(char* buf, int max_buf_size, const UIntType* rads, unsigned int base) {
  ...
  /**
   * @brief Garner's algorithm to compute multipliers for mixed radix conversion.
   * When converting a number in modulo form to mixed radix representation
   * (prod_i=0^{k-1} m_i)^{-1} mod m_k need to be computed.
   * They need to be computed only once, hence are provided here as separate function.
