...
struct mod {
  static uint64_t const VALUE
    = (VAL>=0)?static_cast<uint64_t>(VAL)%MOD
...
class prodmod {
  ...
  static uint64_t const
    P2 = (P > MOD/2)?MOD-(MOD-P)-(MOD-P):(P<<1);
  prodmod(); // dont instantiate
public:
  static uint64_t const VALUE = (1-Y%2)*P2
    + (Y%2)*((P2 <= (MOD-X))*(MOD-(MOD-X-P2)) + (P2 > (MOD-X))*(P2-(MOD-X)));
};
...
public:
  static uint64_t const
    POWER = ((1-(N&1))*((two_powers<N/2>::POWER)+1));
};
template<>
struct two_powers<1> {
...
struct powermod {
  ...
  static uint64_t const VALUE = (1-(P&1))*P2
      + (P&1)*prodmod<N,P2,MOD>::VALUE;
private:
...
template<>
class is_prime<0,0> { public:
  static uint64_t const prim = 1;
};

template<>
class is_prime<0,1> { public:
  static uint64_t const prim = 1;
};
