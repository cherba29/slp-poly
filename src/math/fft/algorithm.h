 ...
 */
template <typename F>
void polyDivMod(unsigned int lCoeffs, const F* left, unsigned int rCoeffs,
                const F* right, F* remquo);
...
class Fourier {
 ...
 public:
   ...
   * computes coefficients of their product % x^nCoeffs
   */
  void polyProduct(unsigned int nCoeffs, const F* left, const F* right,
                   F* prod);
  ...
  /**
   * Given polynomials left (of degree < lCoeffs) and right (of degree <
   * rCoeffs), divides left by right and returns coeffs of remainder followed by
   * coeffs of quotient in the array remquo
   */
  void polyDivMod(unsigned int lCoeffs, const F* left, unsigned int rCoeffs,
                  const F* right, F* remquo);
};
