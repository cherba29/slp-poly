...
void inverse(unsigned int logsize, const F* points, F* result) {
  ...
  ASSERT0(fSize.hasInverse()).msg("Does not have inverse in math::fft::inverse");
...
void polyInvert(unsigned int nCoeffs, const F* poly, F* result) {
  ...
  ASSERT1(poly[0].hasInverse())
    .msg("Passed in polynomial has no invertible constant term, "
  ...
  unsigned int newDeg;
  for (unsigned int oldDeg = 1, i = math::intlog(nCoeffs);
        i > 0; i--, oldDeg = newDeg) {
...
template <typename F>
void polyDivMod(unsigned int aNTerms, const F* a,
                unsigned int bNTerms, const F* b, F* remquo)
{
  unsigned int i;
  ...
  for (; i < qNTerms; ++i) { tmp[i].template setTo<0>();  }

  polyInvert<F>(qNTerms, tmp, bRevInverse); // compute inverse of rev b

  // Calculate quotient
  for (i = 0; i < qNTerms; i++) {  tmp[i] = a[aNTerms-1-i];  } // rev a

  polyProduct<F>(qNTerms, tmp, bRevInverse, tmp);

  for (i = 0; i < qNTerms; i++) { remquo[bNTerms-1+i] = tmp[qNTerms-1-i];  }
  ...
  // tmp <- b
  for (i = 0; i < bNTerms; ++i) { tmp[i] = b[i];  }
  for (; i < aNTerms; ++i) { tmp[i].template setTo<0>(); }

  // prod <- q
  F* prod = new F[aNTerms]; // qCoeffs == lCoeffs - rCoeffs+1
  for (i = 0; i < qNTerms; i++) { prod[i] = remquo[bNTerms-1+i]; }
  for (; i < aNTerms; ++i) { prod[i].template setTo<0>(); }
