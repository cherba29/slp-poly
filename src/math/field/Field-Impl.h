...
Field<T>::Field(typename Field<T>::sbase_type v) {
  val_ = (v < 0) ?
    (modulus + v)
  : ((static_cast<unsigned>(v) < modulus)?
      v
      : (v-modulus)
    );
}
...
//*****************************************************************************
template <typename T> __forceinline
Field<T>& Field<T>::operator+=(const Field<T>& right) {
  // Ensure no overflow
...
//*****************************************************************************
template <typename T> __forceinline
void Field<T>::setTo(typename Field<T>::sbase_type v) {
  if (v < 0) {
...
bool Field<uint64_t>::is<0>() const {  return val_ == 0; }

template <> template <>  __forceinline
bool Field<uint32_t>::is<0>() const {  return val_ == 0; }

template <> template <>  __forceinline
bool Field<uint64_t>::is<1>() const {  return val_ == 1; }

template <> template <>  __forceinline
bool Field<uint32_t>::is<1>() const {  return val_ == 1; }

template <> template <> __forceinline
bool Field<uint64_t>::is<-1>() const { return val_ == modulus-1; }

template <> template <> __forceinline
bool Field<uint32_t>::is<-1>() const { return val_ == modulus-1; }

template <typename T> __forceinline
bool Field<T>::is(typename Field<T>::sbase_type val) const {
  return (val>=0)?(val_==val%modulus):(val_==(modulus-((-val)%modulus)));
}

template <typename T> __forceinline
bool Field<T>::hasInverse() const {  return !is<0>(); }
//*****************************************************************************
template <typename T> __forceinline
Field<T> operator+(const Field<T>& left, const Field<T>& right) {
  Field<T> A;
...
Field<uint64_t> operator*(const Field<uint64_t>& left, const Field<uint64_t>& right) {
...
template <> __forceinline
Field<uint32_t> operator*(const Field<uint32_t>& left, const Field<uint32_t>& right) {
...
//*****************************************************************************
template <> __forceinline
Field<uint32_t>& Field<uint32_t>::operator*=(const Field<uint32_t>& right) {
  uint64_t q = val_;
  q *= right.val_;
...
//#ifdef __LP64__
template <> __forceinline
Field<uint64_t>& Field<uint64_t>::operator*=(const Field<uint64_t>& right) {
  uint128_t q = val_;
  q *= right.val_;
...
//*****************************************************************************
template <typename T> __forceinline
void Field<T>::add(Field<T>* dest, const Field<T>& src1, const Field<T>& src2) {
...
//*****************************************************************************
template <typename T> __forceinline
void Field<T>::subtract(Field<T>* dest, const Field<T>& src1, const Field<T>& src2) {
...
//*****************************************************************************
template <typename T> __forceinline
void Field<T>::neg(Field<T>* dest, const Field<T>& src) {
  if (src.val_) {
...
#ifdef __LP64__
template <> __forceinline
void Field<uint64_t>::multiply(Field<uint64_t>* dest, const Field<uint64_t>& src1, const Field<uint64_t>& src2) {
...
#endif
template <> __forceinline
void Field<uint32_t>::multiply(Field<uint32_t>* dest, const Field<uint32_t>& src1, const Field<uint32_t>& src2) {
...
//*****************************************************************************
template <typename T> __forceinline
void Field<T>::divide(Field<T>* dest, const Field<T>& src1, const Field<T>& src2) {
  Field<T>::multiply(dest, src1, src2.getInverse());
}
//*****************************************************************************
template <typename T> __forceinline
Field<T> Field<T>::getPow(unsigned int p) const {
  Field<T> A;
...
//*****************************************************************************
template <> __forceinline
Field<uint32_t>& Field<uint32_t>::sqr() {
  uint64_t q = val_;
...
#ifdef __LP64__
template <> __forceinline
Field<uint64_t>& Field<uint64_t>::sqr() {
  uint128_t q = val_;
  BOOST_STATIC_ASSERT(sizeof(q) >= (sizeof(val_)<<1));
  q *= val_;
  val_ = q % modulus;
...
//*****************************************************************************
template <> __forceinline
Field<uint32_t>& Field<uint32_t>::invert() {
  //val = 1/val;
  uint32_t c = modulus;
  uint32_t d = val_;
  //int c1 = 1, d1 = 0;
...
template <> __forceinline
Field<uint64_t>& Field<uint64_t>::invert() {
  //val = 1/val;
  uint64_t c = modulus;
  uint64_t d = val_;
  //int c1 = 1, d1 = 0;
...
//*****************************************************************************
template <typename T> __forceinline
Field<T>& Field<T>::powerBy(unsigned int p) {
    if (p == 1) { return *this; }
...
//*****************************************************************************
template <typename T> __forceinline
void Field<T>::pow(Field<T>* dest, const Field<T>& src, unsigned int p) {
    if (p == 1) {
      dest->val_ = src.val_;
      return;
    }
...
//*****************************************************************************
template <typename T> __forceinline
Field<T>* Field<T>::allocSq(unsigned int size) {
  return new Field<T>[size*size];
}
//*****************************************************************************
template <typename T> __forceinline
Field<T>* Field<T>::allocSq2p(unsigned int size) {
...
template <> __forceinline
unsigned int getFourierOrder<Field<uint32_t> >() {
  static unsigned int order = math::field::getPrimeFourierOrder(Field<uint32_t>::getModulus());
  return order;
}

template <> __forceinline
unsigned int getFourierOrder<Field<uint64_t> >() {
  static unsigned int order = math::field::getPrimeFourierOrder(Field<uint64_t>::getModulus());
  return order;
}
...
void Field<T>::setPrime(typename Field<T>::ubase_type mod) {
  ASSERT0(maybe_prime<typename Field<T>::ubase_type>(mod))(mod)
    .msg("Passed in value is not a prime");
  Field<T>::modulus = mod;
  Field<T>::pos = modulus >> 1;
  //*(const_cast<base_type*>(&modulus)) = mod;
  ubase_type const fourierOrder = getFourierOrder<Field<T> >();
  ASSERT1(fourierOrder > 15)(fourierOrder)
  ...
  computePrimitiveRoots<Field<T> >(Field<T>(-1), primRoots.get(), primRootInvs.get(), fourierOrder);
}
...
/* This introduces dependence on boost::format and std::string
  ...
  ASSERT1(str.size() < Field32::getMaxStringRepLength())
    ...
    .msg("Element cannot be represented in max established length for string representation. Something went really wrong!");
...
//    (val_ <= pos) ? static_cast<long long>(val_) : -static_cast<long long>(modulus-val_));
//#else
