...
//** Copy constructor **********************************************************
template <typename UIntType> __forceinline
CRT<UIntType>::CRT(const CRT<UIntType>& right)
  : vals_(new ubase_type[CRT<UIntType>::nPrimes_]) {
  for (unsigned int i = 0; i < nPrimes_; i++) {
...
void CRT<uint32_t>::setTo<0>()  {
  for (unsigned int i = 0; i < nPrimes_; ++i) { vals_[i] = 0; }
}
...
__forceinline void CRT<uint32_t>::setTo<1>()  {
  for (unsigned int i = 0; i < nPrimes_; ++i) { vals_[i] = 1; }
}
...
template <> template <>
__forceinline void CRT<uint32_t>::setTo<-1>()  {
  for (unsigned int i = 0; i < nPrimes_; ++i) { vals_[i] = primes_[i] - 1; }
}
template <> template <>
__forceinline void CRT<uint64_t>::setTo<-1>()  {
  for (unsigned int i = 0; i < nPrimes_; ++i) { vals_[i] = primes_[i] - 1; }
...
//******************************************************************************
template <typename UIntType>
__forceinline void CRT<UIntType>::setTo(typename CRT<UIntType>::sbase_type val) {
  if (val >= 0) {
    for (unsigned int i = 0; i < nPrimes_; i++) {
      vals_[i] = val % (primes_[i]);
    }
...
__forceinline void CRT<UIntType>::random() {
  for (unsigned int i = 0; i < nPrimes_; ++i) {
    vals_[i] = math::random<typename CRT<UIntType>::ubase_type>::get() % primes_[i];
  }
}
//******************************************************************************
template <typename UIntType> __forceinline
void CRT<UIntType>::invertableRandom() {
...
//******************************************************************************
template <typename UIntType> __forceinline
bool CRT<UIntType>::is(typename CRT<UIntType>::sbase_type val) const {
if (val>=0) {
    for (unsigned int i = 0; i < nPrimes_; i++)
      if (vals_[i] != val%primes_[i]) return false;
    return true;
...
//******************************************************************************
template <typename UIntType> __forceinline
bool CRT<UIntType>::isPositive() const {
...
//************************************************************************
template <typename UIntType> __forceinline
bool CRT<UIntType>::isNegative() const {
...
//******************************************************************************
template <typename UIntType> __forceinline
bool CRT<UIntType>::hasInverse() const {
  for (unsigned int i = 0; i < nPrimes_; ++i) {
    if (vals_[i] == 0) { return false;  }
  }
  return true;
...
//### Modifiers ################################################################
template <typename UIntType> __forceinline
void CRT<UIntType>::negate() {
  for (unsigned int i = 0; i < nPrimes_; ++i) {
    if (vals_[i] != 0) { vals_[i] = primes_[i] - vals_[i]; }
  }
}

//******************************************************************************
template <typename UIntType> __forceinline
CRT<UIntType>& CRT<UIntType>::swap(CRT<UIntType>& f) {
...
//******************************************************************************
template <typename UIntType> __forceinline
CRT<UIntType>& CRT<UIntType>::invert() {
...
//******************************************************************************
template <typename UIntType> __forceinline
void CRT<UIntType>::add(CRT* dest,
                        const CRT& src1,
                        const CRT& src2) {
  *dest = src1;
  *dest += src2;
...
//******************************************************************************
template <typename UIntType> __forceinline
void CRT<UIntType>::subtract(CRT* dest,
                             const CRT& src1,
                             const CRT& src2) {
  *dest = src1;
  *dest -= src2;
...
//******************************************************************************
template <typename UIntType> __forceinline
void CRT<UIntType>::neg(CRT* dest, const CRT& src) {
  *dest = src;
  dest->negate();
}
...
//******************************************************************************
template <typename UIntType> __forceinline
void CRT<UIntType>::multiply(CRT* dest,
                             const CRT& src1,
                             const CRT& src2) {
  *dest = src1;
  *dest *= src2;
...
//******************************************************************************
template <typename UIntType> __forceinline
void CRT<UIntType>::divide(CRT<UIntType>* dest,
...
//******************************************************************************
template <typename UIntType> __forceinline
void CRT<UIntType>::pow(CRT<UIntType>* dest,
...
//******************************************************************************
template <typename UIntType> __forceinline
bool operator==(const CRT<UIntType>& left, const CRT<UIntType>& right) {
...
template <typename UIntType> __forceinline
CRT<UIntType> operator+(const CRT<UIntType>& left, const CRT<UIntType>& right) {
...
//******************************************************************************
template <typename UIntType> __forceinline
CRT<UIntType> CRT<UIntType>::operator-() const {
...
//******************************************************************************
template <typename UIntType> __forceinline
CRT<UIntType>& CRT<UIntType>::operator=(const CRT<UIntType>& right) {
  for (unsigned int i = 0; i < nPrimes_; i++) {
    vals_[i] = right.vals_[i];
  }
  return *this;
...
CRT<UIntType>* CRT<UIntType>::alloc(unsigned int size) {
  ...
  for (unsigned int i = 0; i < size; ++i) {
    buff[i].vals_ = reinterpret_cast<ubase_type*>(
        cBuff + size * sizeof(CRT<UIntType>)
...
//******************************************************************************
template <typename UIntType> __forceinline
void CRT<UIntType>::dealloc(CRT<UIntType>* buff) {
  delete[] reinterpret_cast<char*>(buff);
}

//******************************************************************************
template <typename UIntType> __forceinline
CRT<UIntType>* CRT<UIntType>::allocSq(unsigned int size) {
  return CRT<UIntType>::alloc(size*size);
}

//******************************************************************************
template <typename UIntType> __forceinline
CRT<UIntType>* CRT<UIntType>::allocSq2p(unsigned int size) {
  ...
  char* cBuff = new char[nPrimes_ * sizeof(ubase_type)
                         * size * size + sizeof(CRT<UIntType>) * sz2p2];
...
void CRT<UIntType>::setPrimes(const ubase_type* prs, int n) {
  ...
  for (unsigned int i = 1; i < nPrimes_; ++i) {
    ...
    for (unsigned int j = i; j > 0; --j) {
      prod = ModOp<ubase_type>::mul(
        prod, primes_[j-1], primes_[i]);
    }
  ...
  computePrimitiveRoots<CRT<UIntType> >(-1, primRoots.get(), primRootInvs.get(), fourierOrder);
}
...
template <typename UIntType>
typename CRT<UIntType>::ubase_type*
CRT<UIntType>::mixedRadix() const {
  ubase_type* rads = new ubase_type[nPrimes_];
  ubase_type eval;
  unsigned int i, j;
  for (i = 0; i < nPrimes_; ++i) { rads[i] = vals_[i]; }
  for (i = 1; i < nPrimes_; ++i) {
    eval = rads[i-1];
    for (j = i-1; j > 0; --j) {
      eval = ModOp<ubase_type>::mul_add(eval, primes_[j-1], rads[j-1], primes_[i]);
    ...
    rads[i] = ModOp<ubase_type>::sub_mul(rads[i], eval, mixmuls_[i-1], primes_[i]);
  }
  return rads;
}
...
int CRT<UIntType>::toString(char* buffer) const {
  ...
  for (i = nPrimes_ - 1;  i >= 0; --i) {
    UIntType half = primes_[i] >> 1;
    if (rads[i] > half) { negated = true; break; }
    if (rads[i] < half) { negated = false; break; }
  }
