  ...
  if hasattr(rule.attr, attr):
    for src in getattr(rule.attr, attr):
      for f in src.files.to_list():
        if not (f.path.startswith('external/') or f.path.startswith('bazel-out/')):
...
def _collected_cpp_files_aspect_impl(target, ctx):
  ...
  return [
    CollectedCppFilesInfo(
      files = depset(collected_cpp_files,
      transitive=collected_cpp_files_deps)
    )
  ]
